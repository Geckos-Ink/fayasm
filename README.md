...
- Reference opcodes `ref.null`, `ref.is_null`, and `ref.func` now execute, and `call_indirect` now performs table lookup + signature validation + trap paths.
- Funcref `null` and function index `0` currently share the same in-table raw representation, so indirect-calling function `0` from tables needs a follow-up encoding fix.
