...
- Standard flow:
  ...
  cmake --build .
  ```
- `./build.sh` automates the rebuild: it attempts to build `wasm_samples/` fixtures (`emcc` preferred, `rustc --target wasm32-unknown-unknown` fallback), nukes `build/`, configures with the same flags, builds shared + static libraries, compiles `fayasm_test_main`, and executes it.
