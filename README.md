...
- The runtime now exposes optional function traps plus spill/load hooks for JIT microcode programs and linear memory, enabling SD-backed offload on ESP32-class devices, and it can bind imported functions to host callbacks or dynamic libraries (via `dlopen`/`dlsym`).
...
- Host import binding is now scaffolded (callback registration or `dlopen`/`dlsym`), but imported tables/memories and full host ABI ergonomics still need refinement.
