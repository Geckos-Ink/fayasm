...
- `fa_jit.*` introduces scaffolding for microcode JIT planning: system probes, budget/advantage scoring, runtime-fed decoded opcode streams, and preparation of per-function microcode programs reused across calls.
...
- **JIT planning (`fa_jit.*`)**: Lightweight scaffolding that probes resources, computes budgets/advantage scores, prepares microcode programs from decoded opcode streams, and executes prepared-op sequences for microcode preparation and WASM-to-microcode conversion.
...
- `src/`
  ...
  - `fa_jit.*` – JIT planning utilities (probes, budgets, per-function microcode program preparation + execution from runtime-fed opcode streams).
