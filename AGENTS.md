...
- `src/fa_ops.*` - opcode descriptors plus the delegate table; microcode scaffolding now pre-stacks function pointer sequences for bitwise/bitcount/shift/rotate plus compare/arithmetic/convert and float unary/special/reinterpret/select ops, per-op handlers now use those microcode functions instead of switch-case towers (gated by a RAM/CPU probe; override via `FAYASM_MICROCODE`), reference opcodes (`ref.null`/`ref.is_null`/`ref.func`) are wired with funcref encode/decode validation for `table.*` and `call_indirect`, and the file now includes section-level/macro-family comments that should be maintained with behavior changes.
