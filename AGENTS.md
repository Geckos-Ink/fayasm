...
- `wasm_samples/` - fixture sources (`.c` for Emscripten, `.rs` for Rust fallback) plus `wasm_samples/build.sh` that generates standalone `.wasm` modules for runtime smoke tests; `typed_values.{c,rs}` covers parameterized exports (i32/i64 args via `fa_Runtime_executeJobWithArgs`) and i64-typed returns.
...
3. Expand runtime smoke coverage using `wasm_samples/` modules.
