...
- `src/fa_runtime.*` - execution entry points, allocator hooks, call-frame management, operand stack reset, locals initialization, call-argument transfer from operand stack into callee locals for wasm-to-wasm calls, top-level argument injection via `fa_Runtime_executeJobWithArgs`, linear memory provisioning (multi-memory/memory64), multi-value returns, loop label unwinding (params when present, otherwise results), label arity checks, imported-global overrides via `fa_Runtime_setImportedGlobal`, host import bindings for functions + imported memories/tables (callbacks/buffers or dynamic libraries) with `fa_RuntimeHostCall_*` ABI helpers, encoded funcref handling (`null` = `0`, function index `n` = `n + 1`) across runtime/host ref carriers, live rebind propagation for imported memories/tables after module attach, optional per-function trap hooks, spill/load hooks for JIT programs and memory, JIT cache eviction bookkeeping, plus per-function JIT opcode caches (optionally pre-scanned with `FAYASM_JIT_PRESCAN` or forced with `FAYASM_JIT_PRESCAN_FORCE`) that feed microcode preparation, precompile passes, and dispatch.
...
- `src/fa_wasm.*` - disk or in-memory parser for module sections (types, functions, exports, globals, memories, tables, elements, data segments), including typed element-expression payloads (`ref.func`/`ref.null`/`global.get`) for funcref/externref tables.
